            ...
            */
            ...
            if (joystickStaysInFixedPosition == false)
            {
                ...
                if (unNormalizedInput.magnitude > inputVector.magnitude)
                {
                    ...
                    currentPosition.x = Mathf.Clamp(currentPosition.x, 0 + bgImage.rectTransform.sizeDelta.x, Screen.width / (2f));
                    currentPosition.y = Mathf.Clamp(currentPosition.y, 0, Screen.height - (bgImage.rectTransform.sizeDelta.y * (1.5f)));

                    // moves the entire joystick along with the drag  
                    bgImage.rectTransform.position = currentPosition;
                  //  if (Input.touchCount == 0)
                    //{
                //        m_RectTransform.anchoredPosition = new Vector2(530f,104f);
                        //Debug.Log(m_RectTransform.anchoredPosition.x);
                    //}
                }
            }
        }
